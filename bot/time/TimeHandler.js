...
class TimeHandler {
  constructor(edge) {
    ...
    async init() {
        ...
        for (const file of files) {
            ...
            if (global.config.time[pull.name] === undefined) await this.edge.editConfig(`time/${pull.name}`, false)
            ...
            pull.start = new cron.CronJob(
                ...
                async function() {
                    ...
                    eventsClass.running[pull.name] = pull.run(eventsClass.edge, {}).catch(async (e) => {
        ...
        console.cron(`${running}/${this.events.size} Time Events`);
        let events_2 = []
        events.forEach(n => events_2.push(this.run(n)))
        events = await Promise.all(events_2);
        if (running) console.cron('All time events are done')
    }
  ...
  async run(pull, options = {}) {
    if (global.config.time[pull.name] && pull.onstart === true && this.ready[pull.name]) {
      ...
      this.running[pull.name] = pull.run(this.edge, options).catch(async (e) => {
        let err = await console.error(e, 'Time Event ERROR')
        global.channels?.log?.send({ embeds: [err] })
      });
