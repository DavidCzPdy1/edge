...
module.exports = {
    name: 'rakety-hlasovani',
    description: 'Creates new question!',
    permissions: [{ id: '378928808989949964', type: 'USER', permission: true}, { id: '1122998678425174016', type: 'ROLE', permission: true}],
    guild: ['1122995611621392424'],
    options: [
      {
        name: 'question',
    ...
    run: async (edge, interaction) => {
      ...
      let events = await edge.get('rakety', 'events', {}).then(n => n.filter(a => a._id.toLowerCase() == data.question.toLowerCase()))
      ...
      for (let answer of data.answers.split('|')) {
        odpovedi.addComponents(new ButtonBuilder().setCustomId(`rakety-hlasovani_cmd_select_${data.question}_${answer}`).setStyle(2).setLabel(answer).setDisabled(true))
        data[answer] = []
      }

      let accept = new ActionRowBuilder()
        .addComponents(new ButtonBuilder().setCustomId(`rakety-hlasovani_cmd_accept_${data.question}`).setStyle(3).setLabel('POSLAT'))
        .addComponents(new ButtonBuilder().setCustomId(`rakety-hlasovani_cmd_deny_${data.question}`).setStyle(4).setLabel('NEPOSLAT'))
        
    
      data._id = data.question
      delete data.question

      await edge.post('rakety', 'events', data)
    ...
    select: async (edge, interaction) => {
      ...
      let data = await edge.get('rakety', 'events', {_id: question})
    ...
    deny: async (edge, interaction) => {
      ...
      await edge.delete('rakety', 'events', {_id: question})
    ...
    accept: async (edge, interaction) => {
      ...
      let event = await edge.get('rakety', 'events', {_id: question})
      ...
      for (let answer of event.answers.split('|')) {
        let styl = 2
        odpovedi.addComponents(new ButtonBuilder().setCustomId(`rakety-hlasovani_cmd_select_${event._id}_${answer}`).setStyle(styl).setLabel(answer).setDisabled(false))
      }

      let message = await channel.send({ embeds: [getEmbed(event, {guild: interaction.guild})], components: [odpovedi], content: `[<@&1128290333386608662>]`, allowedMentions: { parse: ['roles']} })
      event.msgUrl = message.url
      event.message = message.id

      await edge.post('rakety', 'events', event)
    },
    dochazka: async (edge, interaction) => {
      ...
      let data = await edge.get('rakety', 'treninky', {_id: _id})
