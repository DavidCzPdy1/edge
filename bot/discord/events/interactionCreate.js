...
module.exports = async (edge, interaction) => {
    ...
    else if (cID?.split('_')[1] == 'cmd' && edge.commands.get(cID.split('_')[0])) try { edge.commands.get(cID.split('_')[0])[cID.split('_')[2]] (edge, interaction) } catch (e) {console.error(e).then((n) => interaction.reply({ embeds: [n], ephemeral: true}).catch((a) => interaction.followUp({ embeds: [n], ephemeral: true}).catch((o) =>{console.error(o)})))}
